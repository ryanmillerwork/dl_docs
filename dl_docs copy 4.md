...
Brief
    A loop that iterates over each element in a list, assigning the
    element's value to `varName` and executing a Tcl script.
...
Returns
    • The result of the last command evaluated in the final iteration.
...
Example
    ...
    set result [dl_slist]
    dl_foreach letter $letters {
        dl_append $result "letter-$letter"
    }
    dl_tcllist $result
...
Brief
    ...
    `step` (default 1.0), and stopping at or before `stop`.

Inputs
    • start … The starting numeric value.
    • stop …… The ending numeric value.
    • step …… Optional numeric increment (default: 1.0).
...
Example
    ...
    dl_tcllist [dl_fromto 0 1 0.25]
    # → 0.0 0.25 0.5 0.75 1.0

See also
    dl_ilist, dl_flist, dl_fill

=== dl_gaussian2D (Creation / Image Processing) ===
Synopsis
    dl_gaussian2D <rows> <cols> <sigma>

Brief
    [WARNING: This command is broken.]
    It is intended to create a 2D Gaussian kernel, but it fails with a
    segmentation fault.
...
Returns
    • Crashes the `essctrl` process.

Example
    # This command will cause a segmentation fault.
    # dl_gaussian2D 10 10 1.5
...
Returns
    • The value of the element at the specified index. If the element is a
      sublist, it returns the name of that sublist.

Errors
    • TCL_ERROR if the index is out of bounds.
...
Inputs
    • Operands … Two numeric DynLists or a list and a scalar.

Returns
    • A new integer list of 0s and 1s.

Example
    dl_tcllist [dl_gt [dl_ilist 1 5 3] [dl_ilist 2 2 3]]
    # → 0 1 0
...
Brief
    Performs an element-wise greater than (>) comparison and returns the
    indices where the result is true.

Inputs
    • Operands … Two numeric DynLists or a list and a scalar.

Returns
    • A new integer list of 0-based indices.

Example
    dl_tcllist [dl_gtIndex [dl_ilist 1 5 3] 2]
    # → 1 2

See also
    dl_ltIndex, dl_eqIndex, dl_gteIndex

=== dl_gte (Logical / Comparison) ===
Synopsis
    dl_gte <list_or_val1> <list_or_val2>

Brief
    Performs an element-wise greater than or equal to (>=) comparison.
...
Example
    dl_tcllist [dl_gte [dl_ilist 1 5 3] [dl_ilist 2 2 3]]
    # → 0 1 1

See also
    dl_lte, dl_gt, dl_eq

=== dl_gteIndex (Logical / Indexing) ===
Synopsis
    dl_gteIndex <list_or_val1> <list_or_val2>

Brief
    Performs an element-wise greater than or equal to (>=) comparison and
    returns the indices where the result is true.
...
Example
    dl_tcllist [dl_gteIndex [dl_ilist 1 5 3] 3]
    # → 1 2

See also
    dl_lteIndex, dl_gtIndex, dl_eqIndex
...
Brief
    Provides help information for DynList commands. If no command is
    specified, it lists all available commands.

Inputs
    • command_name … An optional string with the name of a `dl_` command.

Returns
    • A string or Tcl list of strings containing help text.

Example
    dl_help dl_add
    # → usage: dl_add list1 list2 [list3...]

    dl_help
    # → (A long list of all available commands)
...
Synopsis
    dl_hist <data_list> <nbins>

Brief
    [WARNING: This command's behavior is unusual.]
    It computes a histogram of the data list but returns a list containing
    two sublists: the first appears to be bin counts, and the second is
    the input `nbins` repeated `nbins` times.
...
Returns
    • A list of lists containing histogram counts and repeated nbins.

Example
    set data [dl_flist 1 1 2 2 2 3 4 5]
    dl_tcllist [dl_hist $data 4]
    # → {2 3 1 2} {4 4 4 4}

See also
    dl_bins, dl_histLists
...
Brief
    Computes a histogram of a data list using predefined bin edges.

Inputs
    • data_list … A numeric DynList.
    • bin_edges_list … A numeric DynList of bin edges.

Returns
    • A new integer list of bin counts.

Example
    set data [dl_flist 1 1 2 2 2 3 4 5]
    set edges [dl_flist 0 2 4 6]
    dl_tcllist [dl_histBins $data $edges]
    # → 2 4 2
...
Brief
    [WARNING: This command is broken.]
    It is intended to compute histograms for a list of lists but causes
...
=== dl_idiff (Arithmetic / Data Analysis) ===
Synopsis
    dl_idiff <list> [lag]

Brief
    Computes the in-place element-wise difference between lagged elements
    of a list (`list[i] = list[i] - list[i-lag]`).

Inputs
    • list … A 1D numeric DynList to be modified in-place.
    • lag …… An optional positive integer (default: 1).
...
Inputs
    • valueN … A sequence of numeric values. Floats will be truncated.

Returns
    • The name of a new integer DynList.

Example
    dl_tcllist [dl_ilist 1 5 9.9]
    # → 1 5 9
...
Synopsis
    dl_increment <list_name>

Brief
    Increments each element of a numeric list by 1 in-place.

Inputs
    • list_name … The name of a numeric DynList to modify.
...
=== dl_index (Accessing Elements) ===
Synopsis
    dl_index <list_name> <index>

Brief
    An alias for `dl_get`. Retrieves the element at a specific 0-based index.
...
Brief
    Inserts a value into a list at a specified index. Modifies the list in-place.

Inputs
    • list_name … Name of the DynList to modify.
    • index ……… 0-based integer index for insertion.
...
Brief
    Converts elements of a source list to integer type, returning a new list.

Inputs
    • source_list … A numeric DynList or a Tcl list of numbers.

Returns
    • The name of a new integer DynList.

Example
    dl_tcllist [dl_int [dl_flist 1.1 2.9 3.5]]
    # → 1 2 3
...
Synopsis
    dl_interleave <list1> <list2>

Brief
    [WARNING: This command is broken.]
    It is intended to interleave two lists but causes a segmentation fault.
...
=== dl_irand (Creation) ===
Synopsis
    dl_irand <count> <max_val>

Brief
    Creates a new integer list of random numbers.

Inputs
    • count … The number of random integers to generate.
    • max_val … The exclusive upper bound for the random numbers (0 to max_val-1).
...
Brief
    [WARNING: This command is misnamed and does not find a position.]
    It returns a new integer list with a `1` at the last position and `0` elsewhere.

Inputs
    • list_name … The name of any existing DynList.

Returns
    • A new integer DynList of the same length as the input list.
...
Returns
    • An integer count of elements.

Example
    dl_length [dl_slist a b c]
    # → 3
...
Inputs
    • list_of_lists_name … The name of a list of lists.

Returns
    • A new integer DynList of sublist lengths.

Example
    set lol [dl_llist [dl_ilist 1 2] [dl_ilist 3 4 5]]
    dl_tcllist [dl_lengths $lol]
    # → 2 3

See also
    dl_length, dl_llength
...
Brief
    An alias for `dl_length`. It works on both simple lists and lists of lists.

See also
    dl_length, dl_lengths

=== dl_llist (Creation / List of Lists) ===
Synopsis
    dl_llist [list_name1 list_name2 ...]

Brief
    Creates a new list of lists.

Inputs
    • list_nameN … A sequence of existing DynList names.

Returns
    • The name of a new list-of-lists DynList.

Example
    set l1 [dl_ilist 1 2]
    set l2 [dl_slist a b]
    set lol [dl_llist $l1 $l2]
    dl_tcllist [dl_get $lol 0]
    # → 1 2

See also
    dl_ilist, dl_flist, dl_slist
...
Brief
    Computes the element-wise natural logarithm (ln).

Inputs
    • Operands … A numeric DynList or scalar. Values must be > 0.

Returns
    • A new float list.

Example
    dl_tcllist [dl_log [dl_flist 1 2.718 10]]
    # → 0.0 0.9998... 2.302...
...
Brief
    Computes the element-wise base-10 logarithm.
...
Example
    dl_tcllist [dl_log10 [dl_ilist 1 10 100]]
    # → 0.0 1.0 2.0
...
Example
    dl_tcllist [dl_lt [dl_ilist 1 5 3] 3]
    # → 1 0 0
...
Brief
    Performs an element-wise less than (<) comparison and returns the
    indices where the result is true.
...
Example
    dl_tcllist [dl_ltIndex [dl_ilist 1 5 3] 3]
    # → 0

See also
    dl_gtIndex, dl_eqIndex, dl_lteIndex
...
Example
    dl_tcllist [dl_lte [dl_ilist 1 5 3] 3]
    # → 1 0 1
...
Example
    dl_tcllist [dl_lteIndex [dl_ilist 1 5 3] 3]
    # → 0 2

See also
    dl_gteIndex, dl_ltIndex, dl_eqIndex
...
Inputs
    • list_name … The name of a numeric DynList.

Returns
    • The maximum value.

Example
    dl_max [dl_flist -1.5 10.1 5.0]
    # → 10.1
...
Brief
    Computes the arithmetic mean (average) of a numeric list.

Inputs
    • list_name … The name of a numeric DynList.

Returns
    • The float mean value.

Example
    dl_mean [dl_ilist 1 2 3 4]
    # → 2.5

See also
    dl_max, dl_min, dl_sum
...
Brief
    Performs element-wise integer modulus.

Inputs
    • Operands … Two integer DynLists or a list and a scalar.

Returns
    • A new integer list of remainders.

Example
    dl_tcllist [dl_mod [dl_ilist 5 -5 5 -5] 3]
    # → 2 1 2 1
...
=== dl_mult (Arithmetic) ===
Synopsis
    dl_mult <list1> <list2> [<list3>...]

Brief
    Performs element-wise multiplication of two or more lists, or a list and
    a scalar. Returns a new list.

Inputs
    • Operands … Two or more numeric DynLists or scalars.

Returns
    • A new numeric DynList.

Example
    dl_tcllist [dl_mult [dl_ilist 1 2 3] [dl_ilist 10 20 30]]
    # → 10 40 90
...
Brief
    Computes the element-wise negation of a list or number.

Inputs
    • Operands … A numeric DynList or scalar.

Returns
    • A new numeric list.

Example
    dl_tcllist [dl_negate [dl_flist -1.1 1.2]]
    # → 1.1 -1.2
...
Brief
    Performs element-wise logical NOT. Non-zero becomes 0, zero becomes 1.
...
Example
    dl_tcllist [dl_not [dl_ilist 5 0 -1]]
    # → 0 1 0
...
Inputs
    • Operands … Two DynLists or a list and a scalar.

Returns
    • A new integer list of 0s and 1s.

Example
    dl_tcllist [dl_noteq [dl_slist a b c] [dl_slist a d c]]
    # → 0 1 0

See also
    dl_eq

=== dl_noteqIndex (Logical / Indexing) ===
Synopsis
    dl_noteqIndex <list_or_val1> <list_or_val2>

Brief
    Performs an element-wise not equal (!=) comparison and returns the
    indices where the result is true.
...
Example
    dl_tcllist [dl_noteqIndex [dl_slist a b c] [dl_slist a d c]]
    # → 1

See also
    dl_eqIndex

=== dl_oneof (Logical / Searching) ===
Synopsis
    dl_oneof <source_list> <search_list>

Brief
    For each element in the source list, checks if it is present in the
    search list. Returns a list of 0s (false) and 1s (true).

Inputs
    • source_list … The DynList to check.
    • search_list … The DynList containing values to search for.

Returns
    • A new integer list of 0s and 1s.

Example
    set data [dl_slist a b c d a]
    set query [dl_slist a c e]
    dl_tcllist [dl_oneof $data $query]
    # → 1 0 1 0 1
...
Brief
    Creates a new float list of the given count, with all elements set to 1.0.
...
Example
    dl_tcllist [dl_or [dl_ilist 1 0 1 0] [dl_ilist 0 0 1 1]]
    # → 1 0 1 1
...
Synopsis
    dl_orIndex <list1> <list2>

Brief
    Performs element-wise logical OR and returns the indices where true.
...
Example
    dl_tcllist [dl_orIndex [dl_ilist 1 0 0] [dl_ilist 0 1 0]]
    # → 0 1
